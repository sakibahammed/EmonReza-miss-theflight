...
const PDFEditingInterface: React.FC = () => {
  ...
  return (
    <div className="font-display bg-background-light dark:bg-background-dark text-slate-900 dark:text-white overflow-hidden h-screen flex flex-col">
      {/* Top Navigation Bar */}
      <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4 lg:px-6 shrink-0 z-20">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <h1 className="text-slate-900 dark:text-white font-bold text-base tracking-tight">Contract_Draft_v2.pdf</h1>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">Last saved 2 minutes ago</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button className="flex items-center justify-center h-9 px-4 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-sm font-semibold transition-colors gap-2">
            <Icon name="share" className="text-[18px]" />
            <span>Share</span>
          </button>
          <button 
            onClick={() => navigate('/export')}
            className="flex items-center justify-center h-9 px-4 rounded-lg bg-primary hover:bg-blue-700 text-white text-sm font-semibold transition-colors gap-2 shadow-sm"
          >
            <Icon name="download" className="text-[18px]" />
            <span>Export PDF</span>
          </button>
          <div 
            className="w-9 h-9 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden ml-2 border border-slate-300 dark:border-slate-600 bg-cover bg-center"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col relative min-w-0">
          {/* Toolbar (Sticky) */}
          <div className="h-14 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-center gap-2 px-4 shrink-0 z-10 shadow-sm">
            {/* Tool Group: Selection */}
            <div className="flex items-center gap-1 pr-4 border-r border-slate-200 dark:border-slate-800">
              <button className="p-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800" title="Selection Tool">
                <Icon name="arrow_selector_tool" />
              </button>
              <button className="p-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800" title="Hand Tool">
                <Icon name="pan_tool" />
              </button>
            </div>
            {/* Tool Group: Annotation */}
            <div className="flex items-center gap-1 px-4 border-r border-slate-200 dark:border-slate-800">
              <div className="relative group">
                <button className="p-2 rounded-lg bg-primary/10 text-primary hover:bg-primary/20 transition-colors flex items-center gap-1" title="Highlighter">
                  <Icon name="ink_highlighter" filled />
                  <div className="w-2 h-2 rounded-full bg-yellow-400 border border-black/10"></div>
                </button>
              </div>
              <button className="p-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800" title="Underline">
                <Icon name="format_underlined" />
              </button>
              <button className="p-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800" title="Strikethrough">
                <Icon name="format_strikethrough" />
              </button>
            </div>
            {/* Tool Group: Objects */}
            <div className="flex items-center gap-1 pl-4">
              <button className="p-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800" title="Add Text">
                <Icon name="title" />
              </button>
              <button className="p-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800" title="Sticky Note">
                <Icon name="sticky_note_2" />
              </button>
              <button className="p-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800" title="Shapes">
                <Icon name="shapes" />
              </button>
              <button className="p-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800" title="Signature">
                <Icon name="ink_pen" />
              </button>
            </div>
            <div className="ml-auto flex items-center">
              <button className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-lg text-xs font-semibold text-primary hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
          ...
          {/* PDF Canvas Area */}
          <div className="flex-1 overflow-auto bg-background-light dark:bg-[#0f1115] relative p-8 flex justify-center">
            {/* The PDF Page */}
            <div className="w-full max-w-[850px] bg-white text-slate-900 min-h-[1100px] pdf-page-shadow rounded-sm relative selection:bg-primary/20 selection:text-primary mb-20">
              {/* Page Content Simulation */}
              <div className="p-16 flex flex-col h-full font-serif leading-relaxed text-[15px] text-justify text-[#2c2c2c]">
                {/* Header */}
                ...
                <div className="mb-6 relative group/clause">
                  ...
                  {/* Comment Indicator on Side */}
                  <div className="absolute -right-12 top-0 cursor-pointer hover:scale-110 transition-transform">
          ...
          <div className="fixed bottom-6 left-1/2 ml-32 transform -translate-x-1/2 flex items-center bg-white dark:bg-slate-800 rounded-full shadow-lg border border-slate-200 dark:border-slate-700 p-1 z-50">
            <button className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors">
              <Icon name="chevron_left" className="text-[20px]" />
            </button>
            <div className="px-4 text-sm font-medium text-slate-700 dark:text-slate-200 tabular-nums">
              Page 1 of 12
            </div>
            <button className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors">
            ...
            <button className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition-colors" title="Grid View">
